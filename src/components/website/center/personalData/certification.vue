  ...
  export default {
    ...
    methods:{
      ...
      init(){
        ...
        global.axiosGetReq('/userPersonalInfo/detail', obj).then((res) => {
          if (res.data.callStatus === 'SUCCEED') { 
            ...
            this.certiData.birthday = res.data.data.birthday && util.formatDate.format(new Date(res.data.data.birthday)) || util.formatDate.format(new Date('1970-01-01'))
            this.certiData.sex = res.data.data.sex && res.data.data.sex.toString() || '1'
      ...
      uploadFile4(res, file) {
        this.certiData.imageUrl_open_permit = global.qiniuShUrl + file.response.key
      },
